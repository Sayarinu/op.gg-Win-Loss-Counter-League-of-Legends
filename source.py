...
def check_playing(Region, Username):
    if 'is not in an active game' in sess.get('https://'+Region+'.op.gg/summoner/spectator/userName='+Username.replace(' ','+'), headers={'x-requested-with': 'XMLHttpRequest','user-agent': pers_UA,'accept-language': 'en-US,en;q=0.9',}).text:return 0
    else:return 1
        ...
        if check == 0:
            ...
            try:
                ...
                if opgg_update.status_code == 200:
                    ...
                    print("Updated "+Username+"'s op.gg profile! Waiting until next possible Update @ "+datetime.datetime.fromtimestamp(possible_update).strftime('%H:%M:%S'))
                    game_history=[]
                    x_val = 1
                    for x in range(0, 10):
